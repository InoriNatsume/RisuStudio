// @ts-nocheck
// Legacy CBS - risuai 모듈로 대체 예정
import type { Database, character, loreBook, CbsConditions, RisuModule, LLMModel } from './types';

// Svelte store 대체 - 시뮬레이터용
let currentTriggerId = '';
export function setCurrentTriggerId(id: string) { currentTriggerId = id; }
export function getCurrentTriggerId() { return currentTriggerId; }
...
export function registerCBS(arg:CBSRegisterArg) {
    ...
    registerFunction({
        name: 'trigger_id',
        callback: (str, matcherArg, args, vars) => {
            return getCurrentTriggerId() ?? 'null'
        },
        alias: ['triggerid'],
